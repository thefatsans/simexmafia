...
function mapApiOrders(apiOrders: Record<string, unknown>[]): Order[] {
  return apiOrders.map((apiOrder) => ({
    id: apiOrder.id as string,
    userId: apiOrder.userId as string,
    items: (apiOrder.items as Order['items']) || [],
    subtotal: apiOrder.subtotal as number,
    serviceFee: apiOrder.serviceFee as number,
    discount: apiOrder.discount as number,
    total: apiOrder.total as number,
    ...
    completedAt: apiOrder.completedAt as string | undefined,
    coinsEarned: apiOrder.coinsEarned as number,
    discountCode: apiOrder.discountCode as string | undefined,
  }))
}
...
export default function AdminOrdersClient({
  ...
  initialOrders?: Record<string, unknown>[] | null
}) {
  const router = useRouter()
  const { user, isLoading: gateLoading, isReady } = useAdminGate()
  const [orders, setOrders] = useState<Order[]>(() =>
    initialOrders ? mapApiOrders(initialOrders) : []
  )
  ...
  const loadOrders = async () => {
    ...
    try {
      ...
      const convertedOrders = mapApiOrders(apiOrders as Record<string, unknown>[])
  ...
  useEffect(() => {
    if (!isReady || !user || initialOrders) return
  ...
  if ((gateLoading && !initialOrders) || (isLoading && !initialOrders)) {
