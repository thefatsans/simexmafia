...
export default function AdminUsersClient({
  ...
  initialUsers?: AdminUserRow[] | null
  initialTotal?: number
}) {
  ...
  useEffect(() => {
    if (authLoading) {
      if (!initialUsers) setIsLoading(true)
      return
    }
    if (!user) {
    ...
    if (!initialUsers) setIsLoading(false)
  }, [user, router, authLoading, initialUsers])
  ...
  useEffect(() => {
    if (authLoading || !user || !isAdmin(user.email)) return
    if (initialUsers && !debouncedQuery) return
    loadUsers()
  }, [authLoading, user, loadUsers, initialUsers, debouncedQuery])
  ...
  if ((isLoading && !initialUsers) || (authLoading && !initialUsers)) {
