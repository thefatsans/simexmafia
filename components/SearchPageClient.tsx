        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-3">
              ...
              <h1 className="text-4xl font-bold text-white">Suchergebnisse</h1>
            </div>
          ...
          {query && (
            <p className="text-gray-400">
              {sortedProducts.length > 0 ? (
                <>
                  <span className="text-purple-400 font-semibold">{sortedProducts.length}</span>{' '}
                  Ergebnisse gefunden für "
        ...
        {!query ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {sortedProducts.map((product, index) => (
              <div
                key={product.id}
                className={`animate-fade-in-scale animate-delay-${Math.min((index % 4) * 100, 600)}`}
              >
