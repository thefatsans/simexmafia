...
export default function AdminGoofyCoinCashoutsPage() {
  const router = useRouter()
  const { user, isLoading: authLoading } = useAuth()
  const { showSuccess, showError } = useToast()
  const [items, setItems] = useState<CashoutItem[]>([])
  const [statusFilter, setStatusFilter] = useState<CashoutStatus>('pending')
  ...
  const loadItems = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/admin/goofycoins-cashouts?status=${statusFilter}`, {
        credentials: 'include',
        cache: 'no-store',
      })
      if (!res.ok) {
  ...
  const handleAction = async (item: CashoutItem, action: 'complete' | 'reject') => {
    ...
    try {
      const res = await fetch(`/api/admin/goofycoins-cashouts/${item.id}`, {
        method: 'PATCH',
        credentials: 'include',
